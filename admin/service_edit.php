...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        // Kiểm tra slug đã tồn tại chưa (trừ service hiện tại)
        try {
            $check_query = "SELECT id FROM services WHERE slug = :slug AND id != :id";
            ...
            if ($check_stmt->rowCount() > 0) {
                ...
                // Cập nhật service
                $query = "UPDATE services SET name = :name, slug = :slug, description = :description, content = :content, 
                          icon = :icon, image = :image, price_from = :price_from, price_unit = :price_unit, 
                          features = :features, featured = :featured, status = :status, sort_order = :sort_order, 
                          updated_at = NOW() 
                          WHERE id = :id";
                $stmt = $db->prepare($query);
...
<div class="admin-card">
    <div class="admin-card-header">
        <h5><i class="fas fa-edit"></i> Sửa dịch vụ</h5>
    </div>
                        ...
                        <div class="admin-card-body">
                            ...
                            <div class="form-group-admin mb-3">
                                ...
                                <textarea name="description" class="form-control-admin" rows="3" 
                                          placeholder="Mô tả ngắn về dịch vụ"><?php echo htmlspecialchars($service['description']); ?></textarea>
                            </div>
                            
                            <div class="form-group-admin mb-3">
                                <label class="form-label-admin">Nội dung chi tiết</label>
                                <textarea name="content" class="form-control-admin" rows="10" 
                                          placeholder="Nội dung chi tiết của dịch vụ"><?php echo htmlspecialchars($service['content']); ?></textarea>
                        ...
                        <div class="admin-card-body">
                            <div class="form-group-admin mb-3">
                                ...
                                <textarea name="features" class="form-control-admin" rows="6" 
                                          placeholder="Mỗi tính năng một dòng"><?php echo htmlspecialchars($service['features']); ?></textarea>
                    ...
                    <div class="admin-card mb-4">
                        ...
                        <div class="admin-card-body">
                            ...
                            <div class="form-group-admin mb-3">
                                ...
                                <div id="imagePreview" class="mt-3" style="<?php echo $service['image'] ? 'display: block;' : 'display: none;'; ?>">
                                    <?php 
                                    $current_image_url = '';
                                    if ($service['image']) {
                                        if (strpos($service['image'], 'http') === 0) {
                                            $current_image_url = $service['image'];
                                        } elseif (strpos($service['image'], '/') === 0) {
                                            $current_image_url = SITE_URL . $service['image'];
                                        } else {
                                            $current_image_url = SITE_URL . '/uploads/services/' . basename($service['image']);
                                        }
                                    }
                                    ?>
                                    <img id="previewImg" src="<?php echo htmlspecialchars($current_image_url); ?>" alt="Preview" 
                ...
                <div class="col-md-4">
                    ...
                    <div class="admin-card mb-4">
                        ...
                        <div class="admin-card-body">
                            <div class="form-group-admin mb-3">
                                ...
                                <div class="d-flex gap-2">
                                    <input type="number" name="price_from" class="form-control-admin" 
                                           value="<?php echo $service['price_from'] ? htmlspecialchars($service['price_from']) : ''; ?>"
                                           placeholder="0" step="0.01" min="0">
                                    <input type="text" name="price_unit" class="form-control-admin" 
                                           value="<?php echo htmlspecialchars($service['price_unit'] ?: 'đ'); ?>"
                            ...
                            <div class="form-group-admin mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="featured" id="featured" 
                                           <?php echo $service['featured'] ? 'checked' : ''; ?>>
                            ...
                            <div class="form-group-admin mb-3">
                                <label class="form-label-admin">Ngày tạo</label>
                                <input type="text" class="form-control-admin" value="<?php echo date('d/m/Y H:i', strtotime($service['created_at'])); ?>" disabled>
                            </div>
                        </div>
                    </div>
