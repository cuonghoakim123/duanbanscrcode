...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $published_at = $status == 'active' && !$news['published_at'] ? date('Y-m-d H:i:s') : ($news['published_at'] ?? null);
    ...
    if (empty($title)) {
        ...
        try {
            $check_query = "SELECT id FROM news WHERE slug = :slug AND id != :id";
            ...
            if ($check_stmt->rowCount() > 0) {
                ...
                // Cập nhật news
                $query = "UPDATE news SET title = :title, slug = :slug, excerpt = :excerpt, content = :content, 
                          category = :category, image = :image, featured = :featured, status = :status, 
...
<div class="admin-card">
    <div class="admin-card-header">
        <h5><i class="fas fa-edit"></i> Sửa tin tức</h5>
    </div>
                        ...
                        <div class="admin-card-body">
                            ...
                            <div class="form-group-admin mb-3">
                                ...
                                <textarea name="excerpt" class="form-control-admin" rows="3" 
                                          placeholder="Mô tả ngắn về tin tức"><?php echo htmlspecialchars($news['excerpt']); ?></textarea>
                            </div>
                            
                            <div class="form-group-admin mb-3">
                                <label class="form-label-admin">Nội dung</label>
                                <textarea name="content" class="form-control-admin" rows="10" 
                                          placeholder="Nội dung chi tiết của tin tức"><?php echo htmlspecialchars($news['content']); ?></textarea>
                    ...
                    <div class="admin-card mb-4">
                        ...
                        <div class="admin-card-body">
                            <div class="form-group-admin mb-3">
                                ...
                                <div id="imagePreview" class="mt-3" style="<?php echo $news['image'] ? 'display: block;' : 'display: none;'; ?>">
                                    <?php 
                                    $current_image_url = '';
                                    if ($news['image']) {
                                        if (strpos($news['image'], 'http') === 0) {
                                            $current_image_url = $news['image'];
                                        } elseif (strpos($news['image'], '/') === 0) {
                                            $current_image_url = SITE_URL . $news['image'];
                                        } else {
                                            $current_image_url = SITE_URL . '/uploads/news/' . basename($news['image']);
                                        }
                                    }
                                    ?>
                    ...
                    <div class="admin-card mb-4">
                        ...
                        <div class="admin-card-body">
                            ...
                            <div class="form-group-admin mb-3">
                                ...
                                <input type="text" class="form-control-admin" value="<?php echo number_format($news['views']); ?>" disabled>
                            </div>
                            
                            <div class="form-group-admin mb-3">
                                <label class="form-label-admin">Ngày tạo</label>
                                <input type="text" class="form-control-admin" value="<?php echo date('d/m/Y H:i', strtotime($news['created_at'])); ?>" disabled>
                            </div>
                        </div>
                    </div>
